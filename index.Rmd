...
library(scales)
library(neurocStats)
res = neurocStats::get_package_list()
n_packages = nrow(res)
mn_cov = mean(res$code_coverage)
coverage_cutoff_percent = 50
over_pct = res$code_coverage > coverage_cutoff_percent
n_below_50 = sum(!over_pct)
pct_over_50 = mean(over_pct)
pct_over_50 = scales::percent(pct_over_50)

url <- "https://neuroconductor.org/api/downloads"
download_stats <- jsonlite::fromJSON(RCurl::getURL(url))
download_stats <- as_tibble(download_stats)
...
count_tab = dl %>% 
  count(country) %>% 
  arrange(desc(n))
n_dl_countries = nrow(count_tab)
```

```{r main}
maintainers = tibble::tribble(
                 ~maintainer,     ~sex, ~country_residence,
              "Aaron Fisher",   "male",    "USA",
               "Adi Gherman",   "male",    "USA",
      "Alessandra Valcarcel", "female",    "USA",
  "Angel Garcia de la Garza",   "male",    "USA",
    "Athanasia Mo Mowinckel", "female",    "Norway",
          "Brandon Whitcher",   "male",    "UK",
           "Brian B. Avants",   "male",    "USA",
               "Brian Caffo",   "male",    "USA",
            "Dorian Pustina",   "male",    "USA",
      "Elizabeth M. Sweeney", "female",    "USA",
             "Haochang Shou", "female",    "USA",
             "Jacob Maronge",   "male",    "USA",
      "Jean-Philippe Fortin",   "male",    "USA",
               "Jesper Maag",   "male",    "USA",
                 "Jian Kang",   "male",    "USA",
            "John Muschelli",   "male",    "USA",
               "Jon Clayden",   "male",    "UK",
           "Karsten Tabelow",   "male",    "Germany",
             "Martin Wilson",   "male",    "USA",
         "Marton Kolossvary",   "male",    "Hungary",
             "Matt Craddock",   "male",    "UK",
            "Pamela Russell", "female",    "USA",
             "Simon Garnier",   "male",    "USA",
            "Simon Vandekar",   "male",    "USA",
                   "Yi Zhao", "female",    "USA",
                "Yikai Wang",   "male",    "USA"
  )
n_countries = length(unique(maintainers$country_residence))
stopifnot(all(res$maintainer %in% maintainers$maintainer))
n_maintainers = length(unique(res$maintainer))
female = maintainers$sex == "female"
n_female = sum(female)
pct_female = scales::percent(mean(female))
```

The main objectives of this proposal are:

1. improve the stability and usability of the current (N=`r n_packages`) packages in Neuroconductor.
...
1. Each package has a vignette (a tutorial/long-form documentation) and the all packages has a code coverage above `r coverage_cutoff_percent`% (currently `r pct_over_50` meet this criteria).
2. We add at least 10 new packages over the course of the project and get contributions from 20 existing contributors (out of `r n_maintainers`).  Contributions will be defined as release updates to packages through developers GitHub repositories or submission additional packages to release. 
3. Scale the Neuroconductor framework to 200 packages (currently has `r nrow(res)`) and be able to achieve monthly releases, including releases of Docker images.
...
The Neuroconductor platform (https://neuroconductor.org/) [@neuroconductor] was started in 2014 at Johns Hopkins to centralize medical image analysis R packages, similar to the effort of Bioconductor [@bioconductor].  One of the main strengths of Neuroconductor is the developer community, which involves `r n_maintainers` maintainers residing in `r n_countries` countries, which contribute to, use, and advertise for Neuroconductor.   Our plan to strengthen the community is to host a conference call/meetup twice a year to discuss developments with the maintainers, including requesting feedback for ways to improve the platform, the submission process, feature requests, or increasing user/developer engagement.  With respec to users, we have had users in over `r n_dl_countries` countries download packages from Neuroconductor.

```{r}
musch = res %>% 
  filter(grepl("Muschelli", maintainer)) %>% 
  arrange(type, name) %>% 
  mutate(type = ifelse(name %in% c("aal", "alvin", "sri24"), "data", type))
n_muschelli = nrow(musch)
n_muschelli_data = sum(musch$type == "data")
```


Comparatively, the project development team is small and but also less diverse.  The team is currently all white males, 2 US-born and 2 Romanian-born.  Moreover, one of the developers (John Muschelli) contributes a total of `r n_muschelli` packages to the project.  Many of these packages (`r n_muschelli_data`) are data packages, which include image templates or example data sets for analysis, requiring little maintenance.  We would like to increase the number of developers, especially those from different countries, races, and sex.  Currently, out of the `r n_maintainers` maintainers, only `r n_female` are female (`r pct_female`).  
...
Though Neuroconductor can handle the `r n_packages` packages, we wish to grow the community and number of packages.  Whenever a new package is submitted, that package must be checked, along with any packages that depend on that package.  That creates a large number of continuous integration (CI) jobs.  Thus, increasing the number of packages will require either a) more concurrent jobs on cloud systems, or b) the setup of servers for our needs.  Now, we believe the CI systems we employ can handle our needs, but we need to expand above the free, single-user plans, while keeping the dependency structure bookkeeping on our server side.  
...
1. For the `r n_below_50` with insufficient code coverages, make pull requests to the packages (approximately 10 per month) - 6 months.  
...
This Grant was generated with all materials at https://github.com/muschellij2/CZI.
